...
interface Props { date: string; name: string; pada: number; padaElement: string; activeUntil: string; next: string; symbol: string; text: string; }
...
export const DailySkyNakshatraCard = React.forwardRef<HTMLDivElement, Props>(({ date, name, pada, padaElement, activeUntil, next, symbol, text }, ref) => (
  <div ref={ref} className="relative mx-auto flex aspect-[9/16] w-full max-w-[540px] flex-col overflow-hidden bg-[#f4f1eb] p-[7%] text-[#3c352d]" style={{ containerType: "inline-size", backgroundImage: "radial-gradient(circle at 50% 20%, rgba(92,77,102,.18), transparent 42%), radial-gradient(circle at 80% 90%, rgba(212,175,55,.14), transparent 38%)" }}>
    ...
    <div className="flex flex-1 flex-col items-center justify-center text-center">
      ...
      <h2 className="font-serif text-[10cqw] leading-tight text-[#5c4d66]">{name} · {capitalize(symbol)}</h2>
